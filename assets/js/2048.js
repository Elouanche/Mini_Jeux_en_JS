...
// Affiche le highscore initialement
document.getElementById("highScoreElement").innerText = highScore;
...
//permets de définir par où on peut bouger les blocs
...
// Permets de bouger les blocs (numéro)
  ...
  if(directionX !=0) {
    ...
    for (let j=0;j<4;j++) {
      ...
      while ((i<=3 && stepY === 1) || (i>=0 && stepY === -1)) {
        ...
        // Mettre à jour le highScore si le score actuel est plus élevé
        if(score > highScore) {
          highScore = score;
          document.getElementById("highScoreElement").innerText = highScore;
          localStorage.setItem('2048HighScore', highScore); // met à jour le highscore dans localStorage
        }
...
//Fonction pour savoir si le joueur a perdu 
function isGameOver() {
...
//Alerte en cas de défaite/victoire 
